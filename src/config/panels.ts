...
const FULL_PANELS: Record<string, PanelConfig> = {
  ...
  'threat-timeline': { name: 'Threat Timeline', enabled: false, priority: 1 },
  'strategic-posture': { name: 'AI Strategic Posture', enabled: false, priority: 1 },
  forecast: { name: 'AI Forecasts', enabled: false, priority: 1 }, // trial: unlocked on web, locked on desktop
  ...
  'oil-inventories': { name: 'Oil Inventories', enabled: false, priority: 60 },
  ...
  'global-procurement': { name: 'Global Procurement', enabled: false, priority: 1 },
  ...
  'latest-brief': { name: 'Latest Brief', enabled: false, priority: 1 },
  'satellite-fires': { name: '위성 화재 감시', enabled: false, priority: 2 },
  ...
  'energy-crisis': { name: 'Energy Crisis Tracker', enabled: false, priority: 2 },
  ...
  'airline-intel': { name: '항공 운항 정보', enabled: false, priority: 2 },
...
/**
 * KCG fork: 설정 목록에 노출할 패널 allowlist(사장님 지시 07-21 — 미번역·
 * 미동작 패널 전부 목록에서 제거). 순서 = 한국 관제 목적 우선순위(목록 노출
 * 순서로 그대로 쓰인다). 여기 없는 패널은 기능은 남되 추가 목록에 안 보인다.
 * 키를 넣어 부활시키는 패널(위성 화재 등)은 검증 후 여기 다시 올릴 것.
 */
export const KCG_VISIBLE_PANELS: readonly string[] = [
  'map',           // 권역 상황
  'kcg-breaking',  // 실시간 속보
