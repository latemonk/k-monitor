...
export type DeckMapView = 'global' | 'america' | 'mena' | 'eu' | 'asia' | 'latam' | 'africa' | 'oceania' | 'kr_west' | 'kr_south' | 'kr_jeju' | 'kr_east';
...
const VIEW_PRESETS: Record<DeckMapView, { longitude: number; latitude: number; zoom: number }> = {
  // KCG fork: 'global' (the boot/reset view) centers on the Korean peninsula
  // and its surrounding waters — this is a Korea Coast Guard situational
  // display, so "reset view" must always come home to Korean waters.
  global: { longitude: 127.8, latitude: 36.0, zoom: 5.2 },
  kr_west: { longitude: 125.4, latitude: 37.2, zoom: 7 },
  kr_south: { longitude: 128.6, latitude: 34.5, zoom: 7 },
  kr_jeju: { longitude: 126.5, latitude: 33.3, zoom: 7.5 },
  kr_east: { longitude: 130.8, latitude: 37.3, zoom: 6.5 },
...
export class DeckGLMap {
  ...
  private liveTankers: Array<{ mmsi: string; lat: number; lon: number; speed: number; shipType: number; name: string; heading: number; course: number; timestamp: number }> = [];
  private liveTankersAbort: AbortController | null = null;
  private liveTankersTimer: ReturnType<typeof setInterval> | null = null;
  // KCG fork: 관심 대상(선박·항공기) 항적 오버레이 — 레이어 토글과 무관하게
  // 워치리스트가 비어있지 않으면 항상 표시한다 (kcg-watchlist.ts 가 공급).
  private watchOverlay: Array<{ id: string; kind: string; label: string; alerted: boolean; trail: [number, number][] }> = [];
  ...
  // KCG fork: maplibre globe projection (3D) — globe.gl 렌더러 대체.
  // 세션 전용(새로고침=2D 시작): 고줌에서 3D 는 티가 안 나서, 저장해두면
  // 리로드 때 "이도 저도 아닌" 상태로 뜬다(사장님 피드백 07-21).
  private globeProjection = false;

  /** 현재 투영을 maplibre 에 적용(초기 style.load·setStyle 재적용 공용). */
  private applyGlobeProjection(): void {
    const map = this.maplibreMap as unknown as {
      setProjection?: (spec: { type: 'globe' | 'mercator' }) => void;
    } | null;
    try {
      map?.setProjection?.({ type: this.globeProjection ? 'globe' : 'mercator' });
      // 런타임 projection 전환을 deck 오버레이가 못 따라가 아이콘 레이어
      // (선박 화살표 등)가 화면에서 사라진다(2D 데이터가 3D 에서 소실 —
      // 사장님 실측 07-21). 투영이 실제로 바뀐 경우에만 오버레이를 재장착해
      // 카메라 동기화를 강제한다.
      if (this.deckOverlay && this.maplibreMap && this.lastOverlayProjectionGlobe !== this.globeProjection) {
        const control = this.deckOverlay as unknown as maplibregl.IControl;
        this.maplibreMap.removeControl(control);
        this.maplibreMap.addControl(control);
        this.deckOverlay.setProps({ layers: this.buildLayers(true) });
        this.lastOverlayProjectionGlobe = this.globeProjection;
      }
    } catch (err) {
      console.warn('[DeckGLMap] projection switch failed:', err);
    }
  }

  // 3D 진입 전 뷰(2D 복귀 시 복원용)
  private preGlobeView: { center: [number, number]; zoom: number } | null = null;
  // deck 오버레이가 마지막으로 동기화된 투영(재장착 중복 방지)
  private lastOverlayProjectionGlobe: boolean | null = null;
  ...
  private kcgHighlightListener = (e: Event): void => {
    const d = (e as CustomEvent<{ icao24?: string; lat?: number; lon?: number }>).detail;
    if (!d) return;
    this.highlightedAircraft = d.icao24 ?? null;
    if (typeof d.lat === 'number' && typeof d.lon === 'number') {
      try { this.maplibreMap?.easeTo({ center: [d.lon, d.lat], zoom: Math.max(this.maplibreMap.getZoom(), 7), duration: 800 }); } catch { /* ignore */ }
    }
    this.render();
  };
  ...
  private buildKcgVesselCard(d: { mmsi: string; lat: number; lon: number; speed: number; shipType: number; name: string; heading: number; course: number; timestamp: number }): void {
    ...
    const cog = Number.isFinite(Number(d.course)) && Number(d.course) < 360 ? `${Math.round(Number(d.course))}°` : '—';
  ...
  public setGlobeProjection(enabled: boolean): void {
    if (this.globeProjection === enabled) return;
    this.globeProjection = enabled;
    this.applyGlobeProjection();
    // 고줌에서는 globe≈mercator 라 3D 전환이 티가 안 난다("이도 저도 아님" —
    // 사장님 피드백 07-21). 3D 진입 시 지구본이 보이는 줌으로 내려가고,
    // 2D 복귀 시 원래 뷰로 되돌린다.
    ...
    try {
      if (enabled) {
        ...
        this.preGlobeView = { center: [c.lng, c.lat], zoom: map.getZoom() };
        map.easeTo({ zoom: Math.min(map.getZoom(), 2.2), duration: 1200 });
      } else if (this.preGlobeView) {
        map.easeTo({ center: this.preGlobeView.center, zoom: this.preGlobeView.zoom, duration: 900 });
        this.preGlobeView = null;
      }
    } catch (err) {
      console.warn('[DeckGLMap] projection view transition failed:', err);
    }
  }

  public isGlobeProjection(): boolean {
    return this.globeProjection;
  }
  ...
  constructor(container: HTMLElement, initialState: DeckMapState, options: DeckGLMapOptions = {}) {
    ...
    if (this.chrome) {
      this.createControls();
      // KCG fork: news time-range chips removed (meaningless for the ops view
      // and they overlapped the layer picker).
      void this.createTimeSlider; // retained for potential re-enable
  ...
  private applyRadarLayer(): void {
    ...
    try {
      ...
      this.maplibreMap.addSource('weather-radar', {
        ...
        tileSize: 256,
        // RainViewer serves no tiles past ~z10 (returns "Zoom Level Not
        // Supported" placeholders). Cap and let maplibre overzoom.
        maxzoom: 7,
  ...
  private async initMapLibre(): Promise<void> {
    ...
    const recreateWithFallback = () => {
      ...
      // KCG: setStyle/재생성마다 projection 이 스타일 기본값으로 돌아가므로 재적용
      this.maplibreMap.on('style.load', () => this.applyGlobeProjection());
    };
    ...
    this.maplibreMap.on('style.load', () => this.applyGlobeProjection());
    // KCG fork: 지도 중심을 전역으로 노출(공항 현황 거리순 정렬 등에서 사용)
    // + 추적 리스트 하이라이트 이벤트 수신.
    this.maplibreMap.on('moveend', () => {
      const c = this.maplibreMap?.getCenter();
      if (c) (window as unknown as { __kcgMapCenter?: { lat: number; lon: number } }).__kcgMapCenter = { lat: c.lat, lon: c.lng };
    });
    {
      const c0 = this.maplibreMap.getCenter();
  ...
  private buildLayers(deferHeavy = false): LayersList {
    ...
    // KCG fork: 관심 대상 항적 오버레이 — 토글 없이 항상 표시. 마지막에
    // push해야 다른 레이어(conflicts 등)에 가려지지 않는다.
    if (this.watchOverlay.length > 0) {
      layers.push(...this.createWatchOverlayLayers());
    }
    ...
    return new IconLayer<PositionSample>({
      ...
      getColor: (d) => {
        ...
        if (this.highlightedAircraft && d.icao24 === this.highlightedAircraft) {
          return [255, 60, 60, 255] as [number, number, number, number];
        }
      ...
      pickable: true,
      billboard: false,
      updateTriggers: { getColor: this.highlightedAircraft, getSize: this.highlightedAircraft },
    });
  }
  ...
  // KCG fork: 방향 화살표 아이콘(침로 회전) + 선종 색(마린트래픽 스타일).
  // 흰색 마스크 아이콘에 getColor 틴트를 입힌다.
  private static readonly VESSEL_ICON_ATLAS = 'data:image/svg+xml;base64,' + btoa(
    '<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64"><path d="M32 3 L53 58 L32 45 L11 58 Z" fill="#fff"/></svg>',
  );
  ...
  private createLiveTankersLayer(): IconLayer {
    return new IconLayer({
      ...
      iconAtlas: DeckGLMap.VESSEL_ICON_ATLAS,
      iconMapping: { arrow: { x: 0, y: 0, width: 64, height: 64, mask: true, anchorX: 32, anchorY: 32 } },
      getIcon: () => 'arrow',
      getPosition: (d) => [d.lon, d.lat],
      // deck.gl 회전은 반시계 방향(도) — 침로(시계·북=0)를 뒤집는다.
      // 침로가 없으면 선수방위(heading), 그것도 없으면 0.
      getAngle: (d) => {
        const cog = Number(d.course);
        const hdg = Number(d.heading);
        const deg = Number.isFinite(cog) && cog >= 0 && cog < 360 ? cog
  ...
  private getTooltip(info: PickingInfo): { html: string } | null {
    ...
    switch (layerId) {
      ...
      case 'live-tankers-layer': {
        // 선박 상세 카드 — 선명/국적/MMSI/선종/속력/침로/마지막 수신.
        const flag = flagFromMmsi(obj.mmsi);
        const kn = Number.isFinite(Number(obj.speed)) ? `${Number(obj.speed).toFixed(1)} kn` : '—';
        const cog = Number.isFinite(Number(obj.course)) && Number(obj.course) < 360 ? `${Math.round(Number(obj.course))}°` : '—';
        const seen = Number.isFinite(Number(obj.timestamp))
          ? new Date(Number(obj.timestamp)).toLocaleTimeString(navigator.language || 'ko-KR', { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false })
          : '—';
        return {
          html: `<div class="deckgl-tooltip"><strong>${text(obj.name || '(선명 미상)')}</strong><br/>`
            + `선종 : ${text(shipTypeKo(obj.shipType))}<br/>`
            + `국적 : ${flagEmoji(flag.iso)} ${text(flag.nameKo)}<br/>`
            + `속력 : ${kn}<br/>`
            + `침로 : ${cog}<br/>`
            + `MMSI : ${text(obj.mmsi)}<br/>`
            + `<span style="opacity:.65">마지막 수신 ${seen}</span></div>`,
        };
      }
  ...
  private createLayerToggles(): void {
    ...
    // KCG fork: '선박 교통'(ais)과 '실시간 선박'(liveTankers)이 사용자에겐
    // 중복으로 보임(사장님 피드백 07-21) — 화살표를 그리는 liveTankers 만
    // 노출하고 ais 는 목록에서 숨긴다(스트림 사이드이펙트·상태는 유지).
    const layerDefs = getLayersForVariant((SITE_VARIANT || 'full') as MapVariant, 'flat')
      .filter(def => def.key !== 'ais');
    ...
    // KCG fork: 레이어 패널 하단 원저작자 배지 비표시(사장님 지시 07-21).
    // 저작권·AGPL 고지는 레포 LICENSE 에 그대로 유지된다.
  ...
  private showLayerExplanation(layer: keyof MapLayers): void {
    ...
    // KCG fork: 닫는 방법이 안 보인다는 피드백(07-21) — ESC·바깥 클릭·우상단
    // ✕ 어느 쪽으로도 닫히게 한다.
    ...
    const onOutside = (e: MouseEvent): void => {
      const target = e.target as HTMLElement;
      if (popup.contains(target) || target.closest('.layer-explain-btn')) return;
      closePopup();
    ...
    if (!popup.querySelector('.layer-explanation-close')) {
      const x = document.createElement('button');
      x.className = 'layer-explanation-close';
      x.textContent = '✕';
      x.setAttribute('aria-label', '닫기');
      x.style.cssText = 'position:absolute;top:8px;right:10px;background:none;border:none;color:#9ab;font-size:15px;cursor:pointer;';
      x.addEventListener('click', closePopup);
      popup.style.position = popup.style.position || 'absolute';
      popup.prepend(x);
    }
    this.container.appendChild(popup);
  ...
  /** KCG fork: 관심 대상 항적 오버레이 데이터 교체 (kcg-watchlist 이벤트 수신). */
  public setWatchOverlay(items: Array<{ id: string; kind: string; label: string; alerted: boolean; trail: [number, number][] }>): void {
    this.watchOverlay = items;
    this.render();
  }

  /** 관심 대상 항적(PathLayer) + 현재 위치 링(Scatterplot) + 라벨(Text). */
  private createWatchOverlayLayers(): Array<PathLayer | ScatterplotLayer | TextLayer> {
    const color = (w: { kind: string; alerted: boolean }): [number, number, number, number] =>
      w.alerted ? [231, 60, 60, 235]
        : w.kind === 'aircraft' ? [255, 209, 102, 220] : [0, 209, 255, 220];
    const heads = this.watchOverlay
      .filter((w) => w.trail.length > 0)
      .map((w) => ({ ...w, pos: w.trail[w.trail.length - 1]! }));
    return [
      new PathLayer({
        id: 'kcg-watch-trails',
        data: this.watchOverlay.filter((w) => w.trail.length >= 2),
        getPath: (w: { trail: [number, number][] }) => w.trail,
        getColor: color,
        getWidth: 2,
        widthUnits: 'pixels',
        capRounded: true,
      ...
      new ScatterplotLayer({
        id: 'kcg-watch-heads',
        data: heads,
        getPosition: (w: { pos: [number, number] }) => w.pos,
        getFillColor: [0, 0, 0, 0],
        getLineColor: color,
        getRadius: 9,
        radiusUnits: 'pixels',
      ...
      new TextLayer({
        ...
        data: heads,
        getPosition: (w: { pos: [number, number] }) => w.pos,
        getText: (w: { label: string }) => `★ ${w.label}`,
        getColor: color,
        getSize: 12,
        sizeUnits: 'pixels',
        getPixelOffset: [0, -18],
