...
export class EventHandlerManager implements AppModule {
  ...
  private setupMissionPresets(): void {
    ...
    // KCG fork: never auto-open the workspace chooser — this is a fixed
    // situational display; operators can still open it from the header button.
    const shouldPrompt = false as boolean &&
      !this.ctx.isMobile &&
  ...
  /**
   * KCG fork: bulk map-layer swap for watch-preset tabs. Mirrors the mission
   * preset layer pipeline (renderer filter → persist → map update → per-layer
   * side effects → freshness sync → data refresh) without touching panels,
   * view, or time range — the tab system owns panels.
   */
  applyWorkspaceMapLayers(layers: MapLayers): void {
    const mapLayers = this.filterMissionLayersForCurrentRenderer({ ...layers });
    ...
    // 전체 loadAllData 는 안 태운다 — 모든 호출자(탭 전환/생성/삭제 폴백)가
    // applyTabPanelState 로 이미 스케줄한다(중복 이중 리로드 방지). 개별 레이어
    // 데이터는 위 transitions 의 loadDataForLayer 가 처리.
    this.syncUrlState();
  }
  ...
  startHeaderClock(): void {
    ...
    if (!el) return;
    // KCG fork: user-locale local time (OS timezone), not UTC.
    const fmt = new Intl.DateTimeFormat(navigator.language || 'ko-KR', {
  ...
  // KCG fork: 지도 핀 버튼 제거(효과 불명·무반응 — 사장님 지시 07-21).
  // 이름은 배선 호환을 위해 유지하고 전체보기·3D 토글만 세팅한다.
  setupMapPin(): void {
    const mapSection = document.getElementById('mapSection');
  ...
  // KCG fork: 3D = globe.gl 렌더러 전환이 아니라 maplibre globe projection.
  // DeckGL 렌더러가 유지되므로 선박 화살표 등 deck 레이어가 3D에서도 그대로다.
  ...
  private setupMapFullscreen(mapSection: HTMLElement): void {
    ...
    const toggle = () => {
      ...
      btn.title = isFullscreen ? '지도 전체보기 해제' : '지도 전체보기';
      this.syncMapAfterLayoutChange();
    };
