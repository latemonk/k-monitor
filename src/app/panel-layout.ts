...
import { KCG_WATCH_PRESETS, getKcgWatchPreset } from '@/services/kcg-presets';
...
// KCG fork: 3D(maplibre globe projection)는 deck 데이터 레이어(선박 화살표·
// 분쟁 면)가 globe 투영에서 소실/파편화되는 문제(07-21 실측 — billboard·
// 오버레이 재장착으로도 미해결, deck.gl 9.2 한계로 판단)로 임시 비활성.
// 코드 경로는 보존 — deck 업그레이드 후 true 로 되살릴 것.
const KCG_3D_TOGGLE_ENABLED = false;
...
export interface PanelLayoutManagerCallbacks {
  ...
  /** Bulk map-layer swap for tab switches / watch presets (side effects included). */
  applyMapLayersBulk?: (layers: MapLayers) => void;
}
...
export class PanelLayoutManager implements AppModule {
  ...
  async renderLayout(): Promise<void> {
    // KCG fork: 2D/3D 토글 = maplibre globe projection (globe.gl 렌더러 은퇴,
    // 세션 전용이라 부트는 항상 2D 활성)
    ...
    setTrustedHtml(this.ctx.container, trustedHtml(`
      ...
      <div class="header">
        <div class="header-left">
          ...
          <span class="logo">🇰🇷 대한민국 AI 통합 관제 시스템</span><span class="logo-mobile">🇰🇷 대한민국 AI 관제</span><span class="version">v${__APP_VERSION__}</span>${BETA_MODE ? '<span class="beta-badge">BETA</span>' : ''}<a class="version" href="https://github.com/latemonk/k-monitor" target="_blank" rel="noopener" title="오픈소스예요 — 소스 코드 보기(AGPL-3.0)" style="text-decoration:none">소스</a>
          ${this.ctx.isMobile ? '' : '<div class="dashboard-tabs-mount dashboard-tabs-inline" id="panelTabsMount"></div>'}
          ...
          <div class="region-selector">
            <select id="regionSelect" class="region-select" aria-label="${t('header.selectRegion')}">
              <option value="global">한반도 전역</option>
              <option value="kr_west">서해(NLL·인천)</option>
              <option value="kr_south">남해·대한해협</option>
              <option value="kr_jeju">제주 해역</option>
              <option value="kr_east">동해·독도</option>
              <option value="asia">동아시아</option>
            </select>
          </div>
        ...
        ${this.ctx.isMobile ? '' : '<span class="header-clock" id="headerClock" translate="no"></span>'}
      ...
      <nav class="mobile-menu" id="mobileMenu">
        <div class="mobile-menu-header">
          <span class="mobile-menu-title">해양경찰 통합상황판</span>
      ...
      <main id="main" tabindex="-1" class="main-content${this.ctx.isDesktopApp ? ' desktop-grid' : ''}">
        <div class="map-section${mapStartsCollapsed ? ' collapsed' : ''}" id="mapSection">
          ${this.ctx.isMobile ? `<div class="panel-header">
            <div class="panel-header-left">
              <div class="dashboard-tabs-mount dashboard-tabs-inline" id="panelTabsMount"></div>
            </div>
            <span class="header-clock" id="headerClock" translate="no"></span>
          </div>` : ''}
          <div class="map-container" id="mapContainer"></div>
          ${this.ctx.isMobile ? '' : `<div class="map-overlay-actions">
            ${KCG_3D_TOGGLE_ENABLED ? `<div class="map-dimension-toggle" id="mapDimensionToggle">
              <button class="map-dim-btn${isGlobeMode ? '' : ' active'}" data-mode="flat" title="2D 지도">2D</button>
              <button class="map-dim-btn${isGlobeMode ? ' active' : ''}" data-mode="globe" title="3D 지구본">3D</button>
            </div>` : ''}
            <button class="map-btn" id="mapFullscreenBtn" title="지도 전체보기">
  ...
  private initPanelTabs(): void {
    ...
    if (!state) {
      ...
      // KCG fork: 이 기능 이전에 저장된 탭(mapLayers 없음)은 로드 시점의 라이브
      // 레이어(=사용자의 실제 현재 구성)로 스탬프. 안 하면 프리셋 탭에서 갈아탄
      // 레이어가 스냅샷으로 눌어붙어 원래 구성이 유실된다.
      let stampedLayers = false;
      for (const tab of state.tabs) {
        if (!tab.mapLayers) {
          tab.mapLayers = { ...this.ctx.mapLayers };
          stampedLayers = true;
        }
        // 한글화 이전에 만든 탭 이름 치유(New Tab/Main → 새 탭/기본)
        if (tab.name === 'New Tab') { tab.name = '새 탭'; stampedLayers = true; }
        if (tab.name === 'Main') { tab.name = '기본'; stampedLayers = true; }
        // 실시간 속보 도입 전 탭에 위젯 주입(사용자가 끈 탭은 키가 남아 재주입 안 됨)
        if (!('kcg-breaking' in tab.panelSettings)) {
          tab.panelSettings['kcg-breaking'] = { name: '실시간 속보', enabled: true, priority: 1 };
          const mapIdx = tab.panelOrder.indexOf('map');
      ...
      try {
        if (!localStorage.getItem('kcg-layers-v37')) {
          for (const tab of state.tabs) {
            const preset = getKcgWatchPreset(tab.kcgPreset);
            if (preset) {
              const layers = { ...DEFAULT_MAP_LAYERS };
              for (const key of Object.keys(layers) as Array<keyof MapLayers>) layers[key] = false;
              for (const key of preset.layers) layers[key] = true;
              tab.mapLayers = layers;
            } else {
              tab.mapLayers = { ...DEFAULT_MAP_LAYERS };
            }
          }
          localStorage.setItem('kcg-layers-v37', '1');
          stampedLayers = true;
        }
      } catch { /* storage unavailable — 이번 세션은 라이브 상태 그대로 */ }
      if (stampedLayers) saveTabsState(state);
    }
    this.tabsState = state;
    // KCG fork: 부팅 시 활성 탭의 프리셋을 전역 신호로 공표(브리프·속보 위젯 구독)
    const bootTab = state.tabs.find((tab) => tab.id === state.activeTabId);
    setActiveKcgPreset(bootTab?.kcgPreset ?? null);
  ...
  private captureCurrentTabState(): Pick<PanelTab, 'panelSettings' | 'panelOrder' | 'bottomSet' | 'mapLayers'> {
  ...
  private switchToTab(tabId: string): void {
    ...
    if (target.mapLayers) this.callbacks.applyMapLayersBulk?.(target.mapLayers);
    setActiveKcgPreset(target.kcgPreset ?? null);
    this.panelTabBar?.refresh();
  }

  /**
   * KCG fork: `+` opens the watch-preset picker (해양/공중/지상) instead of
   * silently cloning the default panel set. A trailing plain option keeps
   * the old "default panels" path available.
   */
  private addTab(): void {
    if (!this.tabsState) return;

    const body = document.createElement('div');
    body.className = 'kcg-preset-choices';

    const makeChoice = (icon: string, name: string, desc: string, extraClass?: string): HTMLButtonElement => {
      const btn = document.createElement('button');
      btn.type = 'button';
      btn.className = `kcg-preset-choice${extraClass ? ` ${extraClass}` : ''}`;
      const iconEl = document.createElement('span');
      iconEl.className = 'kcg-preset-choice-icon';
      iconEl.textContent = icon;
      const textWrap = document.createElement('span');
      const nameEl = document.createElement('span');
      nameEl.className = 'kcg-preset-choice-name';
      nameEl.textContent = name;
      const descEl = document.createElement('span');
      descEl.className = 'kcg-preset-choice-desc';
    ...
    let close: () => void = () => {};
    for (const preset of KCG_WATCH_PRESETS) {
      const btn = makeChoice(preset.icon, preset.name, preset.description);
      btn.addEventListener('click', () => {
        close();
        this.createTabFromPreset(preset);
      });
    }
    const plainBtn = makeChoice('＋', '기본 구성', '지금 쓰는 기본 패널 구성으로 빈 탭을 만들어요.', 'kcg-preset-choice-plain');
    plainBtn.addEventListener('click', () => {
      close();
      this.createDefaultTab();
    });

    close = showKcgModalNode('새 감시 탭 만들기', body, 480);
  }
  ...
  private createDefaultTab(): void {
    ...
    setActiveKcgPreset(null);
    this.panelTabBar?.refresh();
    showToast(t('dashboardTabs.newTabCreated'));
  }

  private createTabFromPreset(preset: KcgWatchPreset): void {
    if (!this.tabsState) return;
    this.snapshotActiveTab();

    const built = applyPresetDefinitionToState(
      { panels: preset.panels, layers: preset.layers },
      this.ctx.panelSettings,
    );
    const tab: PanelTab = {
      id: generateTabId(),
      name: preset.name,
      panelSettings: enforceFreePanelLimit(built.panelSettings, isProUser()),
      panelOrder: built.panelOrder,
      bottomSet: [],
      mapLayers: built.mapLayers,
      kcgPreset: preset.id,
    };
    this.tabsState.tabs.push(tab);
    this.tabsState.activeTabId = tab.id;
    ...
    this.applyTabPanelState(tab.panelSettings, tab.panelOrder, tab.bottomSet);
    this.callbacks.applyMapLayersBulk?.(built.mapLayers);
    setActiveKcgPreset(preset.id);
    this.panelTabBar?.refresh();
    showToast(`${preset.name} 탭을 만들었어요`);
  }
  ...
  private deleteTab(tabId: string): void {
    ...
    if (wasActive) {
      ...
      if (fallback.mapLayers) this.callbacks.applyMapLayersBulk?.(fallback.mapLayers);
      setActiveKcgPreset(fallback.kcgPreset ?? null);
    } else {
      saveTabsState(this.tabsState);
    }
    this.panelTabBar?.refresh();
  ...
  private createNewsPanelWithLabel(
    ...
    this.lazyImportedPanel(panelKey, () => import('@/components/NewsPanel'), 'NewsPanel', (NewsPanel) => {
      ...
      // KCG fork: 국내 뉴스 패널 주제 칩(정치/경제/사회·해양/북한/재난/지역)
      const kcgTopics = getKcgNewsTopics(categoryKey);
      if (kcgTopics) panel.enableKcgTopicFilter(kcgTopics);
  ...
  private async createPanels(): Promise<void> {
    ...
    // KCG fork: globe.gl 렌더러 은퇴 — 3D 는 DeckGL 위 maplibre globe projection.
    // (옛 저장값 'globe' 는 App.ts 마이그레이션이 kcg-globe-projection 으로 이관)
    ...
    // KCG fork: force the Korea-centered 'global' framing on every renderer
    // at boot (deck.gl already starts there via VIEW_PRESETS; the SVG
    // fallback only re-frames on an explicit setView call).
    if (!this.ctx.isMobile) this.ctx.map.setView('global');
